...
class EvaluacionProyectoInformanteController extends Controller {
...
//traer a los participantes
        ...
        if ($model != NULL) {
            ...
            $html2pdf->Output('evaluacion_proyecto_informante_' . $model->idAlumno->nombre . '.pdf', 'D');
        } else {
    ...
    public function actionCreate($ida, $idp) {
        $model = new EvaluacionProyectoInformante;
        $model->id_alumno = $ida;
        ...
        if ($modelProyecto != NULL && ($modelProyecto->estado_actual == Estado::$PROYECTO_EN_DESARROLLO || Yii::app()->user->checkeaAccesoMasivo(array(Rol::$ADMINISTRADOR, Rol::$SUPER_USUARIO)))) {
            if (isset($_POST['EvaluacionProyectoInformante'])) {
                ...
                if ($model->evaluacionCompleta()) {
                    ...
                    Yii::app()->user->setFlash('error', "La evaluación en el <b>desarrollo del proyecto por el profesor Informante</b> no está completa, favor de completar y luego enviar.");
                    $this->redirect(array('proyecto/view', 'id' => $model->id_proyecto));
                }
            }
    ...
    public function actionUpdate($id) {
        ...
        $model->scenario = "envia";
        $this->performAjaxValidation($model);
        if ($model->id_evaluacion_proyecto_informante_padre == NULL) {
            // Uncomment the following line if AJAX validation is needed
    ...
    public function actionIndex() {
        $dataProvider = new CActiveDataProvider('EvaluacionProyectoInformante');
    ...
    public function actionAdmin() {
        $model = new EvaluacionProyectoInformante('search');
     ...
     * @return EvaluacionProyectoInformante the loaded model
     * @throws CHttpException
     */
    public function loadModel($id) {
        $model = EvaluacionProyectoInformante::model()->findByPk($id);
        if ($model === null)
     ...
     * @param EvaluacionProyectoInformante $model the model to be validated
     */
    protected function performAjaxValidation($model) {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'evaluacion-proyecto-informante-form') {
