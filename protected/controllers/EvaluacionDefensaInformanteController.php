...
class EvaluacionDefensaInformanteController extends Controller {
    ...
    public function actionCalculaNota() {
        $model = new EvaluacionDefensaInformante;
        if (isset($_GET['EvaluacionDefensaInformante'])) {
            $model->attributes = $_GET['EvaluacionDefensaInformante'];
...
//traer a los participantes
        ...
        if ($model != NULL) {
            ...
            $html2pdf->Output('evaluacion_defensa_informante_' . $model->idAlumno->nombre . '.pdf', 'D');
        } else {
    ...
    public function actionCreate($ida, $idp) {
        ...
        if ($modelProyecto != NULL && ($modelProyecto->estado_actual == Estado::$PROYECTO_EN_DESARROLLO||Yii::app()->user->checkeaAccesoMasivo(array(Rol::$ADMINISTRADOR, Rol::$SUPER_USUARIO)))) {
            ...
            if (isset($_POST['EvaluacionDefensaInformante'])) {
                $model->attributes = $_POST['EvaluacionDefensaInformante'];
    ...
    public function actionUpdate($id) {
        $model = $this->loadModel($id);
        if ($model->id_evaluacion_defensa_informante_padre == NULL) {
            // Uncomment the following line if AJAX validation is needed
    ...
    public function actionIndex() {
        $dataProvider = new CActiveDataProvider('EvaluacionDefensaInformante');
    ...
    public function actionAdmin() {
        $model = new EvaluacionDefensaInformante('search');
     ...
     * @return EvaluacionDefensaInformante the loaded model
     * @throws CHttpException
     */
    public function loadModel($id) {
        $model = EvaluacionDefensaInformante::model()->findByPk($id);
        if ($model === null)
     ...
     * @param EvaluacionDefensaInformante $model the model to be validated
     */
    protected function performAjaxValidation($model) {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'evaluacion-defensa-informante-form') {
    ...
    private function esPropietario($model) {
        ...
        $propietario = FALSE;
        if ($model->idProyecto->prof_informante == Yii::app()->user->id) {
            $propietario = TRUE;
        }
